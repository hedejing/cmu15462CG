...
static void init_sphere()
{
    ...
    for ( int i = 0; i <= SPHERE_NUM_LAT; i++ ) {
        for ( int j = 0; j <= SPHERE_NUM_LON; j++ ) {
            ...
            vptr[TCOORD_OFFSET + 0] = (float)lon;
            vptr[TCOORD_OFFSET + 1] = (1.0f - (float)lat);

            lat *= PI;
            lon *= 2 * PI;
            real_t sinlat = sin( lat );

            vptr[NORMAL_OFFSET + 0] = vptr[VERTEX_OFFSET + 0] = float(sinlat*sin(lon));
            vptr[NORMAL_OFFSET + 1] = vptr[VERTEX_OFFSET + 1] = float(cos(lat)),
            vptr[NORMAL_OFFSET + 2] = vptr[VERTEX_OFFSET + 2] = float(sinlat*cos(lon));
        }
    }
